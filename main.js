...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const parseVideoSource = (rawInput) => {
        if (!rawInput) return { type: 'unknown', url: '' };
        const input = rawInput.trim();

        // Check if raw input is an iframe snippet
            ...
            if (srcMatch) {
                ...
                if (url.includes('youtube.com') || url.includes('youtu.be')) {
                    if (!url.includes('autoplay=')) {
                        url += (url.includes('?') ? '&' : '?') + 'autoplay=1&rel=0&modestbranding=1&playsinline=1';
                    }
        ...
        // Check for YouTube Shorts (9:16 vertical)
        const shortsMatch = input.match(/(?:https?:\/\/)?(?:www\.)?youtube\.com\/shorts\/([a-zA-Z0-9_-]+)/i);
        if (shortsMatch) {
            return {
                type: 'youtube',
                isShort: true,
                url: `https://www.youtube.com/embed/${shortsMatch[1]}?autoplay=1&rel=0&modestbranding=1&playsinline=1`
            };
        }

        // Check for standard YouTube URLs (watch?v=, youtu.be/, embed/)
        const ytMatch = input.match(/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]+)/i);
        if (ytMatch) {
            return {
                type: 'youtube',
                isShort: false,
                url: `https://www.youtube.com/embed/${ytMatch[1]}?autoplay=1&rel=0&modestbranding=1&playsinline=1`
            };
        }
        ...
        if (input.includes('drive.google.com')) {
            let embedUrl = input;
            if (input.includes('uc?export=download&id=')) {
                const id = input.split('id=')[1].split('&')[0];
                embedUrl = `https://drive.google.com/file/d/${id}/preview`;
            } else if (input.includes('/file/d/') && !input.includes('/preview')) {
                embedUrl = input.replace(/\/view.*/, '/preview');
            }
    ...
    // Handle Project Card Clicks (Event Delegation so any newly added cards work instantly)
    document.addEventListener('click', (e) => {
        const card = e.target.closest('.project-card');
        if (!card) return;

        const rawVideo = card.getAttribute('data-video');
        const parsed = parseVideoSource(rawVideo);
        const isExplicitLandscape = card.getAttribute('data-aspect') === '16-9' || card.classList.contains('landscape-card');
        const isLandscape = isExplicitLandscape && !parsed.isShort;
        ...
        if (videoWrapper && parsed.url) {
            ...
            if (parsed.type === 'youtube' || parsed.type === 'iframe' || parsed.type === 'drive') {
                ...
                videoWrapper.innerHTML = `<video id="modal-video" ${isMobile ? 'controls' : ''} autoplay playsinline src="${encodeURI(decodeURI(parsed.url))}"></video>`;
    ...
    // 7. CONTACT FORM SUBMISSION (Real AJAX Email Delivery via Web3Forms)
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', (e) => {
            ...
            const formData = {
                access_key: accessKeyInput ? accessKeyInput.value : "06f92320-e0bb-4d4a-b86f-5ed9c65699a4",
            ...
            .then(data => {
                if (data.success) {
                    ...
                    submitBtn.innerHTML = `<i class="fa-solid fa-triangle-exclamation"></i> ${data.message || 'Error occurred'}`;
                }
    ...
    if (particleCanvas) {
        ...
        class Particle {
            ...
            reset(initial = false) {
                ...
                // Dynamic colors matching red & black portfolio palette
                const randColor = Math.random();
                if (randColor < 0.50) {
                    this.colorType = 'ruby';
                } else if (randColor < 0.85) {
                    this.colorType = 'crimson';
                } else {
            ...
            draw() {
                ...
                let glowColor = 'rgba(239, 35, 60, 0.025)'; // Ruby Red default
                if (this.colorType === 'crimson') {
                    glowColor = 'rgba(153, 27, 27, 0.025)';
                } else if (this.colorType === 'white') {
                    glowColor = 'rgba(255, 255, 255, 0.04)';
                }
                
                const baseGlowAlpha = this.isDragging ? 0.35 : (0.06 + this.glow * 0.15);
                ...
                if (this.colorType === 'ruby') {
                    fillGrad.addColorStop(0, 'rgba(254, 202, 202, 0.30)');
                    fillGrad.addColorStop(0.5, 'rgba(239, 35, 60, 0.14)');
                    fillGrad.addColorStop(1, 'rgba(153, 27, 27, 0.06)');
                } else if (this.colorType === 'crimson') {
                    fillGrad.addColorStop(0, 'rgba(255, 228, 230, 0.28)');
                    fillGrad.addColorStop(0.5, 'rgba(190, 18, 60, 0.14)');
                    fillGrad.addColorStop(1, 'rgba(136, 19, 55, 0.06)');
                } else {
                    fillGrad.addColorStop(0, 'rgba(255, 255, 255, 0.40)');
                    fillGrad.addColorStop(0.5, 'rgba(254, 226, 226, 0.15)');
                    fillGrad.addColorStop(1, 'rgba(225, 29, 72, 0.06)');
                }
